...
async function fetchImgData(index, token){
  ...
  let response = await fetch (`https://www.deviantart.com/api/v1/oauth2/gallery/${randomPickFolder[index].folder}?username=${randomPickFolder[index].artist}&mode=newest&offset=${offsetNum}&limit=20&mature_content=true&access_token=${token}`, {
    method: 'GET',
    headers: {
